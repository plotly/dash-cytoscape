...
- elements (dict; optional): A list of dictionaries representing the elements of the networks. Each dictionary describes an element, and
...
but arrays remain the recommended format. elements has the following type: list of dicts containing keys 'group', 'data', 'position', 'selected', 'selectable', 'locked', 'grabbable', 'classes'.
Those keys have the following types:
  - group (string; optional): Either 'nodes' or 'edges'. If not given, it's automatically inferred.
  - data (dict; optional): Element specific data. data has the following type: dict containing keys 'id', 'label', 'parent', 'source', 'target'.
Those keys have the following types:
...
or `label`. It is only a convention.
  ...
  - position (dict; optional): Only for nodes. The position of the node. position has the following type: dict containing keys 'x', 'y'.
Those keys have the following types:
  - x (number; optional): The x-coordinate of the node.
  - y (number; optional): The y-coordinate of the node.
  - selected (boolean; optional): If the element is selected upon initialisation.
  - selectable (boolean; optional): If the element can be selected.
  - locked (boolean; optional): Only for nodes. If the position is immutable.
  - grabbable (boolean; optional): Only for nodes. If the node can be grabbed and moved by the user.
  - classes (string; optional): Space separated string of class names of the element. Those classes can be selected
by a style selector. | dict containing keys 'nodes', 'edges'.
Those keys have the following types:
  - nodes (list; optional)
  - edges (list; optional)
- stylesheet (dict; optional): A list of dictionaries representing the styles of the elements.
...
string syntax stays the same. stylesheet has the following type: list of dicts containing keys 'selector', 'style'.
Those keys have the following types:
...
[issue](https://github.com/plotly/dash-cytoscape/issues/25) for more information. layout has the following type: dict containing keys 'name', 'fit', 'padding', 'animate', 'animationDuration', 'boundingBox'.
Those keys have the following types:
...
Some external layouts are also included. To use them, run
  ...
  - fit (boolean; optional): Whether to render the nodes in order to fit the canvas.
  - padding (number; optional): Padding around the sides of the canvas, if fit is enabled.
  - animate (boolean; optional): Whether to animate change in position when the layout changes.
  - animationDuration (number; optional): Duration of animation in milliseconds, if enabled.
...
following keys are accepted:. pan has the following type: dict containing keys 'x', 'y'.
Those keys have the following types:
  - x (number; optional): The x-coordinate of the node
  - y (number; optional): The y-coordinate of the node
...
- tapNode (dict; optional): The complete node dictionary returned when you tap or click it. Read-only. tapNode has the following type: dict containing keys 'edgesData', 'renderedPosition', 'timeStamp', 'classes', 'data', 'grabbable', 'group', 'locked', 'position', 'selectable', 'selected', 'style', 'ancestorsData', 'childrenData', 'descendantsData', 'parentData', 'siblingsData', 'isParent', 'isChildless', 'isChild', 'isOrphan', 'relativePosition'.
Those keys have the following types:
  - edgesData (dict; optional): node specific item
  - renderedPosition (dict; optional): node specific item
  - timeStamp (number; optional): node specific item
  - classes (string; optional): General item (for all elements)
  - data (dict; optional): General item (for all elements)
  - grabbable (boolean; optional): General item (for all elements)
  - group (string; optional): General item (for all elements)
  - locked (boolean; optional): General item (for all elements)
  - position (dict; optional): General item (for all elements)
  - selectable (boolean; optional): General item (for all elements)
  - selected (boolean; optional): General item (for all elements)
  - style (dict; optional): General item (for all elements)
  - ancestorsData (dict; optional): Item for compound nodes
  - childrenData (dict; optional): Item for compound nodes
  - descendantsData (dict; optional): Item for compound nodes
  - parentData (dict; optional): Item for compound nodes
  - siblingsData (dict; optional): Item for compound nodes
  - isParent (boolean; optional): Item for compound nodes
  - isChildless (boolean; optional): Item for compound nodes
  - isChild (boolean; optional): Item for compound nodes
  - isOrphan (boolean; optional): Item for compound nodes
...
- tapEdge (dict; optional): The complete edge dictionary returned when you tap or click it. Read-only. tapEdge has the following type: dict containing keys 'isLoop', 'isSimple', 'midpoint', 'sourceData', 'sourceEndpoint', 'targetData', 'targetEndpoint', 'timeStamp', 'classes', 'data', 'grabbable', 'group', 'locked', 'selectable', 'selected', 'style'.
Those keys have the following types:
  - isLoop (boolean; optional): Edge-specific item
  - isSimple (boolean; optional): Edge-specific item
  ...
  - timeStamp (number; optional): Edge-specific item
  - classes (string; optional): General item (for all elements)
  - data (dict; optional): General item (for all elements)
  - grabbable (boolean; optional): General item (for all elements)
  - group (string; optional): General item (for all elements)
...
`'store'` to improve performance by preventing transfer of data to the server. generateImage has the following type: dict containing keys 'type', 'options', 'action', 'filename'.
Those keys have the following types:
  - type (a value equal to: 'svg', 'png', 'jpg', 'jpeg'; required): File type to output
